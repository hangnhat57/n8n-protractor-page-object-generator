...
let LETTERS = new Common().LETTERS;
let VISIBILITIES = new Common().VISIBILITIES;

const ELEMENT_NODE = 1;
const NG_PREFIXES = ['ng-', 'data-ng-', 'ng_', 'x-ng-', 'ng\\:'];
const NG_STRATEGIES = [{handler: getNgModelName, strategy: 'model'}];
...
export class Generator {
    constructor() {
     ...
     generate( document, input){
        ...
        input = input || {};
        let output = Object.extend(input);
        output = common.setDefaultValues(output);
        output = setDefinitions( document,output);
...
// ========================================================================
    ...
    function getComments(document,root) {
        var comments = [];
        var index = -1;
    ...
    function getLabelText(document,node) {
        var text = '';

        if (node && node.id) {
            text = getLabelTextFor(document,node, 'id');
        }

        if (text === '' && node && node.name) {
            // non-standard, but it happens
            text = getLabelTextFor(document,node, 'name');
        }
    ...
    function getLabelTextFor(document,node, attribute) {
    ...
    function getNodeText(document,node) {
        var text = getLabelText(document, node);
    ...
    function getPageVisibleHTML(document,original) {
        original = original || document.body;
        var cloned = original.cloneNode(true);
        cloned = sanitizeNode(document, cloned, original);
    ...
    function sanitizeNode(document,clonedNode, originalNode) {
        var comments = getComments(document,clonedNode);
    ...
    function setDefinitions(document, input) {
        var definitions = [];
        var root = document.querySelector(input.nodes.root) || this.document;
        //var root =  document;
        ...
        if (input.operations.extras['verify.loaded']) {
            var sourceText = getPageVisibleHTML(document);
